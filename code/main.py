...
def main(opt):
    ...
    if opt.task == 'attention':
        ...
        discriminator = Discriminator(total_locations=TOTAL_LOCS)
        gen_train_fixstart = True
    elif opt.task == 'attention_new':
        d_pre_epoch = 10
        g_pre_epoch = 10
        ploss_alpha = float(opt.ploss)
        dloss_alpha = float(opt.dloss)
        generator = ATGeneratorNew(device=device,total_locations=TOTAL_LOCS,starting_sample='real',
                                starting_dist=np.load(f'{DATA_PATH}/{opt.data}/start.npy'),data=opt.data)
        discriminator = Discriminator(total_locations=TOTAL_LOCS)
        gen_train_fixstart = True        
    if opt.debug:
    ...
    for epoch in range(EPOCHS):
        ...
        for it in range(1):
            ...
            inputs = torch.cat([zeros, samples.data], dim=1)[
                              :, :-1].contiguous()
            tim = torch.LongTensor([[i*5%60, i*5//60] for i in range(SEQ_LEN)]).to(device)
        ...
        with open( DATA_PATH+'/%s/logs/loss_%s.log' % (opt.data, opt.task), 'a') as f:
            ...
            f.write('\n')
        if (epoch + 1) % 20 == 0:
            generate_samples(generator, BATCH_SIZE, SEQ_LEN, GENERATED_NUM,  DATA_PATH+'/%s/gene_epoch_%d_%s.data' %
                             (opt.data, epoch + 1, opt.task))
