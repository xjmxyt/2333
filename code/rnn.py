...
def main(opt):
    ...
    DATA_PATH = '../data'
    DATA_WRTING_PATH = '../data1/'+opt.task+'_'+str(opt.batch_size)
    ...
    MODEL_PATH = DATA_WRTING_PATH+'/%s/model_%s.pth'%(opt.data, opt.task)
    TB_LOG_PATH = DATA_WRTING_PATH+'/%s/logs/log_%s'%(opt.data, opt.task) # tensorboard log path

    random.seed(SEED)
    np.random.seed(SEED)
    
    print(DATA_WRTING_PATH)
    if not os.path.exists(DATA_WRTING_PATH):  #判断是否存在文件夹如果不存在则创建为文件夹
        os.makedirs(DATA_WRTING_PATH)
    ...
    for epoch in range(EPOCHS):
        if epoch % 10 == 0:
            ...
            with open( DATA_WRTING_PATH+'/%s/logs/jsd_%s.log' % (opt.data, opt.task), 'a') as f:
        ...
        with open( DATA_WRTING_PATH+'/%s/logs/loss_%s.log' % (opt.data, opt.task), 'a') as f:
            f.write(' '.join([str(j)
