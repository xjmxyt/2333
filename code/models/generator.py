...
from utils import *
from models.embed import TemporalEmbedding, PositionalEmbedding, DataEmbedding
...
class Generator(nn.Module):
    ...
    def __init__(
        ...
        if self.starting_sample == 'real':
            self.starting_dist = torch.tensor(starting_dist).float() # 这个时候从starting_dist中进行采样
...
class ATGenerator(nn.Module):
    ...
    def forward(self, x_l, x_t):
        ...
        mat1 = self.M1[locs]
        mat2 = self.M2[locs]
        mat1 = torch.FloatTensor(mat1).to(self.device)
        mat2 = torch.FloatTensor(mat2).to(self.device)


        lemb = self.loc_embedding(x_l) # (batch_size, seq_len, embed_size)
        temb = self.tim_embedding(x_t)
        x = torch.cat([lemb, temb], dim=-1)
        
        x = x.transpose(0,1) # (seq_len, batch_size, embed_size)
        Query = self.Q(x) # (seq_len, batch_size, hidden_dim)
        Query = F.relu(Query)
        ...
        x, _ = self.attn(Query, Key, Value) # (seq_len, batch_size, E)
        ...
        x = x.reshape(-1, self.linear_dim) # (B*S, H)
        x = self.linear(x)   # (B*S, Loc_num)
        x = F.relu(x)   
...
class ATGeneratorNew(nn.Module):
    """Attention Generator.
    """
    ...
    def __init__(
        ...
        :param starting_dist:
        """
        super(ATGeneratorNew, self).__init__()
        self.total_locations = total_locations
        self.embedding_dim = embedding_dim
        self.hidden_dim = hidden_dim
        self.bidirectional = bidirectional
        ...
        self.embedding = DataEmbedding(1, self.embedding_dim)        
    ...
    def forward(self, x_l, x_t):
        ...
        :param x: (batch_size, seq_len), sequence of locations
        :param t: (batch_size, seq_len, 2) this last dim include [hour, second]
        :return:
            (batch_size * seq_len, total_locations), prediction of next stage of all locations
        """
        locs = x_l.contiguous().view(-1).detach().cpu().numpy()
    ...
    def step(self, l, t):
        ...
        :param x: (batch_size, 1), current location
        :param t: (batch_size, 1, 2), current time
        :param h: (1/2, batch_size, hidden_dim), lstm hidden state
        :param c: (1/2, batch_size, hidden_dim), lstm cell state
        :return:
            (batch_size, total_locations), prediction of next stage
    ...
    def sample(self, batch_size, seq_len, x=None):
        ...
        if flag:
            if s > 0:
                samples.append(x)
            self.init_time_shift(batch_size)
            for i in range(s, seq_len):
                t = self.generate_time(batch_size, i)           
                x = self.step(x,t)
                x = x.multinomial(1)
                samples.append(x)
                torch.floor()
    ...
    def init_time_shift(self, batch_size):
        '''
            让batch中各个样本时间的初始偏移不同
        '''
        self.random_t = torch.randint(0, 1000, (batch_size, 1)).to(self.device)
        
    def generate_time(self, batch_size, step):
        tim = step * 5
        t = torch.LongTensor([tim]).to(self.device)
        t = t.repeat(batch_size).reshape(batch_size, -1)
        # 将随机张量与 t 相加
        t_with_random = t + self.random_t
