...
from models.deepMove import TrajPreLocalAttnLongTraffic
from models.gan_loss import GANLoss, distance_loss, period_loss
from data_iter import TCGenDataIterNew, TCDisDataIter
...
def main(opt):
    ...
    if opt.task == "LSTM" or opt.task == "GRU":
        model = TrajPreLocalAttnLongTraffic(
            device=device,
            num_locs=TOTAL_LOCS,
            loc_emb_size=1000,
            tim_emb_size=48,
    ...
    data_iter = TCGenDataIterNew(REAL_DATA, BATCH_SIZE)
    ...
    for epoch in range(EPOCHS):
        ...
        loss = train_epoch_deepMove(model=model, data_iter=data_iter, criterion=criterion, optimizer=optimizer,
                                    device=device)
